...
import numpy as np
import torch as th
from model import ActivityModel, KmModel, KcatModel
...
if __name__ == "__main__":
    ...
    parser.add_argument("-feat_fpath", type=str, default="train_feats.pkl",
                         help="Input. The path of the kcat/Km dataset.")
    parser.add_argument("-kcat_model_dpath", type=str, default="kcat_model.pth",
                         help="Input. The path of training data.")
    parser.add_argument("-Km_model_dpath", type=str, default="Km_model.pth",
                         help="Input. The path of pretrained model.")
    ...
    for fold in range(10):
        ...
        kcatmodel.load_state_dict(th.load(f"{kcat_model_dpath}/{fold}_bestmodel.pth", map_location=device))
        # load Km model
        Kmmodel = KmModel(device=device)
        Kmmodel.load_state_dict(th.load(f"{Km_model_dpath}/{fold}_bestmodel.pth", map_location=device))
        ...
        valid_pred, valid_label, valid_eval = run_an_eval_epoch(model, valid_dataloader, device)
        out_results(np.array(valid_eval), "results_{}.csv".format(fold))
