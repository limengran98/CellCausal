...
def spearman_pcc(Y, X):
    rX, rY = rankdata(X), rankdata(Y)
    return PCC(rY, rX)

def evaluate(y_true, y_pred):
    _pcc = PCC(y_true, y_pred)
    _scc = spearman_pcc(y_true, y_pred)
...
def write_logfile(epoch, record_data, logfile):
    ...
    columns = ["epoch", "train_pcc", "train_scc", "train_r2", "train_rmse", "train_loss", 
                        "valid_pcc", "train_scc", "valid_r2", "valid_rmse", "valid_loss"]
...
def run_a_training_epoch(model, data_loader, optimizer, device="cuda:0"):
    ...
    for step, data in enumerate(data_loader):
        data = data.to(device)
        ezy_feats = data[:, :1024]
        sbt_feats = data[:, 1024:-2]
        kcat = data[:, -2]
        Km = data[:, -1]
        label = th.log10(kcat/Km)

        if len(label) == 1:
            continue

        _, _, pred_acti = model(ezy_feats, sbt_feats)
        loss = rmse_loss(label, pred_acti.ravel())
        ...
        with th.no_grad():
            ...
            y_pred.append(pred_acti.cpu().detach().numpy().ravel())
...
def run_an_eval_epoch(model, data_loader, device="cuda"):
    ...
    with th.no_grad():
        ...
        return y_pred, y_label, [_pcc, _scc, _r2, _rmse, total_loss/N]
