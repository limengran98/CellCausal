...
def write_experiment_report(trial_dir: str,
    ...
    try:
        ...
        # IMPORTANT:
        # - Phase-2 configs store model/base_url under cfg['llm'].
        # - Phase-3 configs may store providers at top-level (cfg['providers']).
        # If we pass the whole Phase-2 cfg into `chat_text(cfg=...)`, the resolver will NOT see
        # model/base_url (because they are nested), and will fall back to OpenAI defaults.
        # Therefore we route Phase-2 through llm_config=cfg['llm'].

        use_full_cfg = isinstance(cfg, dict) and ("providers" in cfg or "default_provider" in cfg)
        if use_full_cfg:
        ...
        # Persist the full error so users don't have to hunt console output.
        try:
            import traceback

            err_path = os.path.join(trial_dir, "analysis_llm_error_traceback.txt")
            with open(err_path, "w", encoding="utf-8") as f:
                f.write("LLM report generation failed\n")
                f.write(str(e) + "\n\n")
